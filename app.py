# Imports all the librarary we need to us
...
# useing a decorator to rap our GConnect function 
...
# Create a route decorator to show Category function 
...
# Create a route decorator to Display a Specific Item
...
# Create a route decorator to Add a category
...
# Create a route decorator to Edit a category
...
# Create a route decorator to Delete a category function 
...
# Create a route decorator to Add an item
...
# Create a route decorator to Edit an item
...
def edit_Item(category_name, item_name):
    ...
    # If logged in user != item owner redirect them
    if creator.id != login_session['user_id']:
        flash ("You cannot edit thisrestaurants. it is belongs to %s" % creator.name)
        return redirect(url_for('showCatalog'))
    # POST methods
    if request.method == 'POST':
        if request.form['name']:
        ...
        session.commit()
        flash('The cuisine type Successfully Edited!')
...
# Create a route decorator to Delete an item
...
def delete_Item(category_name, item_name):
    ...
    # If logged in user != item owner redirect them
    if creator.id != login_session['user_id']:
        flash ("You cannot delete this restaurant. it is belongs to %s" % creator.name)
        return redirect(url_for('showCatalog'))
    if request.method =='POST':
...
# Create url_for static path processor
